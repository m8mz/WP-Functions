...
then
  ...
  DB_HOST=${wpconn[3]}
    if [[ $1 == '-n' ]]; then
            mysql -h ${DB_HOST} -u ${DB_USER} -p${DB_PASSWORD} -D ${DB_NAME} -e "UPDATE ${PREFIX}options SET option_value = '$2' WHERE option_name = 'siteurl';UPDATE ${PREFIX}options SET option_value = '$2' WHERE option_name = 'home';"
    fi
